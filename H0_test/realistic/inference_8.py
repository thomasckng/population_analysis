...
def realistic_jsd(x, i):
    ...
    p_realistic = np.einsum("ij, j -> ij", plpeak(m, alpha=x[1], mu=x[2], sigma=x[3], w=x[4], mmin=x[5], mmax=x[6], delta=x[7]), DLsq(dL))
...
def scipy_minimize(i):
    return minimize(realistic_jsd,
                    x0=[uniform(10,100), uniform(1.01,5), uniform(10,50), uniform(0.01,10), uniform(0,1), uniform(0.01,10), uniform(50,200), uniform(0.01,10)],
                    bounds = ((10,100), (1.01,5), (10,50), (0.01,10), (0,1), (0.01,10), (50,200), (0.01,10)), args=(i,)).x
...
if __name__ == '__main__':
    ...
    np.savez("result_8_param.npz", result = result, Mz_sample = Mz_sample, realistic_figaro = realistic_figaro)
